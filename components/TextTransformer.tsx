...
const getCodePointDelta = (sample: string, target: string): number =>
...
const TextTransformer: React.FC<TextTransformerProps> = ({
  ...
  const resultText: string = rebuildText(targetText || 'Your Text...', lowerCaseDelta, upperCaseDelta);
  ...
  return (
    <Card>
      ...
      <Card.Actions>
        <Button mode='outlined' onPress={() => {copyToClipboard(resultText, displaySnackbar)}}>
          📋 Copy to Clipboard
        </Button>
      </Card.Actions>
